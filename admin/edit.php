...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!verify_csrf($token)) {
        ...
        try {
            ...
            if ($image) {
                $stmt = $pdo->prepare('UPDATE products SET name=?, slug=?, description=?, price=?, category=?, image=? WHERE id=?');
                $stmt->execute([$name, $slug, $description, $price, $category, $image, $id]);
            } else {
                $stmt = $pdo->prepare('UPDATE products SET name=?, slug=?, description=?, price=?, category=? WHERE id=?');
                $stmt->execute([$name, $slug, $description, $price, $category, $id]);
            }
...
<title>Edit Product | MK Watches</title>
